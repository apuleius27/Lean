...
namespace QuantConnect.Lean.Engine.Results
{
    ...
    public abstract class BaseResultsHandler
    {
        ...
        {
            ProcessLogs(Algorithm.DebugMessages, PacketType.Debug, messageQueueLimit);
            ProcessLogs(Algorithm.ErrorMessages, PacketType.HandledError, messageQueueLimit);
            ProcessLogs(Algorithm.LogMessages, PacketType.Log, messageQueueLimit);
        }

        private void ProcessLogs(ConcurrentQueue<string> concurrentQueue, PacketType packetType, int? messageQueueLimit = null)
        {
            ...
            var endTime = now.AddMilliseconds(250).Ticks;
            string message;
            while (now.Ticks < endTime && concurrentQueue.TryDequeue(out message))
            {
                now = DateTime.UtcNow;
